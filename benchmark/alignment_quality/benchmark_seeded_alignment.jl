...
EXTRA benchmarks: These are done with try catch and are sometimes ignored to due unhandled case of comparing alignments of differing lengths.
    ...
    3. (UNIMPLEMENTED) How well does nw_align/seed_chain_align + cleanup and scaffold strategy recover the "true protein multiple sequence alignment". (EXTRA)
...
two alignments if possible.
    ...
    For (3) (UNIMPLEMENTED) we use the 3 MSAs from (1) - these being the "true" frameshift-free multiple sequence alignment and the MSAs made from nw_align and
...
noised_seqs = LongDNA{4}[mutateSequence(seqs[i], codon_indel_avg=0.0, frameshift_indel_avg = 3.0, sub_mutation_avg=0.0, verbose=false) for i in 1:num_seqs]
...
# Create a histogram
hist = histogram(levenshtein_distances, bins=5, xlabel="AA_levenshtein_distance", ylabel="hits", title="seed_chain_align - protein seq denoising with (Avg = 3)")
display(hist) #may not be displayed correctly if called in repl
savefig("seed_histogram.png")
num_unexpected = 0
println("The following are unexpected mismatches:\n")
# show differences 
for i in 1:num_seqs
    global num_unexpected
...
for idx in 1:num_seqs
    ...
    seq = mutateSequence(seq, codon_indel_avg=0.0, frameshift_indel_avg = 3.0, sub_mutation_avg=0.0, verbose=false)
    ...
    if length(target_alignment[1]) == length(src_alignment[1]) && length(target_alignment[2]) == length(src_alignment[2])
        ...
        if !isapprox(nuc_matched_percentage, 1.0) || !isapprox(prot_matched_percentage, 1.0)
            println("index: ", idx,"\n")
            # run again with verbose
            nuc_matched_percentage = SP_score(target_alignment, src_alignment, verbose=true)
            prot_matched_percentage = SP_score(prot_target_alignment, prot_src_alignment, verbose=true)
...
median_percent = median(filtered_matches) # display these better...
min_match_percent = minimum(filtered_matches) # display these better. 
